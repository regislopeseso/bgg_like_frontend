function modal_Category_Delete_Restore() {
  let self = this;
  self.IsBuilt = false;
  ...
  self.FetchCategoryDetails = () => {
    ...
    $.ajax({
      url: `https://localhost:7081/admins/showcategorydetails?CategoryId=${self.currentCategoryId}`,
      method: "GET",
      xhrFields: {
      ...
      success: function (response) {
        ...
        if (self.isDeleteMode === false) {
          __global.CategoryDeleteRestoreModalController.RenderRestoreMode(
            response.content.categoryName
          );
          self.RemoveContentLoader();
        } else {
          __global.CategoryDeleteRestoreModalController.RenderDeleteMode(
            response.content.categoryName
          );
          self.RemoveContentLoader();
        }
  ...
  // Methods to fill the MODAL BODY with CATEGORY NAME
  self.RenderRestoreMode = (categoryName) => {
    self.ModalBox.removeClass("delete-warning");
    self.ModalBox.addClass("restore-warning");

    // Update the modal title and button text
    self.ModalTitle.html(
      `<h5><span>R</span>estoring the following <span>C</span>ategory</h5>`
    );

    // Fill in the MODAL BODY with the CATEGORY NAME
    self.ModalBody.html(`<h3><span>${categoryName}</span></h3>`);
  };

  self.RenderDeleteMode = (categoryName) => {
    self.ModalBox.removeClass("restore-warning");
    self.ModalBox.addClass("delete-warning");

    // Update the modal title and button text
    self.ModalTitle.html(
      `<h5><span style="color: var(--reddish)">D</span>eleting the following <span style="color: var(--reddish)">C</span>ategory</h5>`
    );

    // Fill in the MODAL BODY with the CATEGORY NAME
    self.ModalBody.html(
      `<h3><span style="color: var(--reddish)">${categoryName}</span></h3>`
    );
  };
  ...
  self.SetUpDelete = () => {
    ...
    $.ajax({
      type: "DELETE",
      url: "https://localhost:7081/admins/deletecategory",
      data: JSON.stringify({
  ...
  self.SetUpRestore = () => {
    ...
    $.ajax({
      type: "POST",
      url: "https://localhost:7081/admins/restorecategory",
      data: JSON.stringify({
        CategoryId: self.currentCategoryId,
      }),
      contentType: "application/json",
